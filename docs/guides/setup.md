...
### Self Hosted. ###
To entirely self-host, you'll need to pull in the font files and let Video.js know where the swf is located. By default, the font files are expected to be in a font directory in the same directory as the CSS file.
If you simply copy the dist folder into your project everything should Just Work™, but the paths can easily be changed by editing the LESS file and re-building, or by modifying the generated CSS file.
