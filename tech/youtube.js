...
_V_.youtube = _V_.PlaybackTech.extend({
  ...
  init: function(player, options){
    ...
    // Don't pass origin if you are working on a localhost server. For some reason, this feature breaks the Youtube API
    // and causes the control bar to lock up. This feature is used to block malicious third-party cross-site injection
    // attacks, but this should not be a concern on a localhost server.
    ...
    _V_.addEvent(parentEl, "techready", _V_.proxy(this, function(){
      ...
      iFrm.src = p + "//www.youtube.com/embed/" + videoId + "?" + this.makeQueryString(params);
